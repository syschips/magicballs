...
**MagicBall** は、最大6人対戦のオンライン対応ボールゲームです。WebRTCによるP2P通信で同期しながらプレイできます。

## コアゲーム機能

### 1. 基本ゲームプレイ
- **最大6人対戦**: 最大6人のプレイヤーが同時対戦（2-6人で設定可能）
- **マップシステム**: 固定パターン1種類（シードで同期）
- **ゲームモード**: クラシック（残機1・レート変動あり）/ パーティ（残機3・レート変動なし）
- **タイム制**: 制限時間内にスコアを競う
- **勝敗判定**: スコアに基づく順位決定
...
- **自動更新**: 自動ポーリングでメッセージ取得
- **NGワードフィルター**: 禁止ワード検出時は本文を置換して保存
...
### 1. スプライトシステム
- **PNGスプライトシート対応**: `imgs/*.png` + `data/spriteSheet.json` のフレーム定義を使用
- **GIFフォールバック**: PNGが読めない場合は同名GIFを静的表示
- **8方向対応**: アイドル/移動時の8方向スプライト（上下左右各2種）
- **レスポンシブスケーリング**: 画面サイズに応じて自動スケール調整（0.6x～1.2x）
- **デバッグモード**: `?debug=on` でスプライトをオフにして図形表示
...
### 1. ゲームデータ
- **結果・レート保存**: `finish.php` でgame_history / rate_historyを更新
- **P2P同期**: WebRTCスナップショットでリアルタイム同期（DBへの状態保存は廃止）
...
### 1. ネットワーク最適化
- **ポーリング間隔調整**: ルーム一覧5秒 / 待機ルーム2秒 / ハートビート3分
- **軽量ペイロード**: 必要最小限のJSON送信（追加圧縮なし）
- **DB接続**: リクエスト単位でPDO接続を生成（接続プールは未実装）
...
### 1. デバッグ機能
- **デバッグモード**: `?debug=on` URLパラメータでスプライトを無効化
- **コンソールログ**: 詳細なデバッグログ
- **状態表示**: ゲーム内部状態の可視化
- **エラートラッキング**: エラーの追跡・記録
- **フォールバック描画**: 画像読み込み失敗時の図形描画
...
- **認証**: セッションベース認証
- **データ同期**: ポーリング + WebRTC DataChannel
